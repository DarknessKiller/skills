...
4. For review context, start with `review-context`; then use targeted `diff --path` or `file` only as needed. Done when the next API call is scoped by PR id, path, ref, or commit id instead of fetching full payloads.
...
- approve: `POST {base}/rest/api/1.0/projects/{PROJECT}/repos/{repo}/pull-requests/{id}/approve?version={version}`
