...
Create isolated work under the repo root, not in a global scratch folder.

## Rules

- Worktree parent: `.worktrees/` at `git rev-parse --show-toplevel`.
- Worktree path: `<branch-or-task-slug>-<YYYYMMDD-HHMMSS>`.
- Ignore rule: add `.worktrees/` to local Git exclude, not shared `.gitignore`, unless the user asks.
- CodeGraph: run `codegraph init <path>` only when the source repo has `.codegraph/` or the user explicitly asks.
...
3. Sanitize the path slug by replacing `/`, spaces, and unusual characters with `-`.
