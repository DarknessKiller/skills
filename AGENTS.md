...
Every `SKILL.md` is either user-invoked (`disable-model-invocation: true` plus `policy.allow_implicit_invocation: false` in `agents/openai.yaml`) or model-invoked (omit both). See `.agents/invocation.md`. Follow `.agents/writing-skills.md` for the weak-model-friendly format: one action per step, one-line completion criteria, inline conditionals, short sentences, at most two levels of nesting. For bundled helper scripts, follow `.agents/writing-scripts.md`: skill-relative paths only, self-contained scripts, agent-safe CLIs.
