...
When writing or editing a `SKILL.md`, follow the format rules in `.agents/writing-skills.md`. These rules make skills usable on weaker models (7B-13B class): one action per step, one-line completion criteria, inline conditionals, short sentences, and at most two levels of nesting. For bundled helper scripts, follow `.agents/writing-scripts.md`: skill-relative paths only, self-contained scripts, agent-safe CLIs.
